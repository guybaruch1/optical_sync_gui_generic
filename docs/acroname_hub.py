...
if __name__ == '__main__':
    led_panel_config_bat_path = r"C:\Users\admin\Downloads\config_tigger_mode.bat"
    hub = AcronameHub()
    print("disconnecting")
    hub.disconnect()
    print(f"hub is{'' if hub.is_connected() else ' NOT'} connected")
    print("connecting")
    hub.try_connect()
    print(f"connected to hub #{hub.serial}")
    print(f"hub is{'' if hub.is_connected() else ' NOT'} connected")

    hub.enable_ports([0], True, delay_in_seconds=0)
    hub.disable_ports([1])
    time.sleep(1)
    subprocess.run(led_panel_config_bat_path, shell=True)

    time.sleep(1)
    hub.enable_ports([1, 6], True, delay_in_seconds=0)
    hub.disable_ports([0])
    time.sleep(1)
    subprocess.run(led_panel_config_bat_path, shell=True)

    print(f"occupied ports: {hub.discover_occupied_ports()}")
    print("disconnecting")
    hub.disconnect()
    print(f"hub is{'' if hub.is_connected() else ' NOT'} connected")
    ...
    #Start The trigger mode of the USB relay
    import serial
    ...
    print(s.read(10))  # see if the board sends anything back
    s.close()
