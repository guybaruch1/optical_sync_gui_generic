...
camera-capture machinery. If tools/dual_panel_diag/diag_baseline_demo_sequence.py DOES
make the panels step but THIS script doesn't, the difference is in
start_scanning's own sequence (see tools/dual_panel_diag/diag_app_sequence_minus_extras.py
next, to isolate exactly which extra step matters).

Run from the repo root: python tools/dual_panel_diag/diag_app_start_scanning.py
