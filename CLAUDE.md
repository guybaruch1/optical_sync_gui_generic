...
The fix, confirmed on real hardware: co-enable the stereo module's **depth** stream alongside IR+RGB, which syncs them regardless of open order. `ContinuousCapture(..., enable_depth_for_ir_sync=True)` (the default) does this via `_depth_sync_stream()` (returns the depth geometry to enable - the first infrared pick's own width/height/fps, `None` for a color+color/Dual-RGB pairing with no stereo module in play, or when the setting is off) and `_build_config(include_depth)`. `start()` probes `config.can_resolve(rs.pipeline_wrapper(pipeline))` before committing to the depth-enabled config and falls back to no-depth if the device can't resolve it at that IR profile, rather than letting `pipeline.start()` throw and kill the run; `depth_sync_active` records which one actually happened so callers (`SessionEngineThread`, `ThresholdPreviewThread`, `StreamPreviewThread`) can warn if the fallback fired.
