...
### `rs.pipeline().start()` can knock the LED panel off a shared USB controller

Real-hardware finding: on one rig, `LED-Panel.exe` commands (`engine/led_panel.py`'s `LEDPanel._run`) worked correctly through Calibration, then started failing with a raw nonzero exit code (`CalledProcessError`, `LED-Panel.exe` itself printing `"No device found."` to its own console) as soon as Threshold Tuning's preview or Live Session's Start ran - every subsequent retry and every subsequent Start click failed identically, with no code-level difference between the working and failing call sites (both call the exact same `dual_panel_control.start_scanning`/`LEDPanel._run` functions, in the same order, with `camera_sync.hardware_reset_before_start` confirmed `false`). A plain `LED-Panel.exe --isRunning` typed directly into a real console succeeded immediately afterward, and disabling `camera_sync.enable_depth_for_ir_sync` (ruling out USB *bandwidth* specifically) made no difference either.

**Root cause, confirmed by physically separating the two devices' USB connections:** the LED panel and the RealSense camera were sharing a USB hub/controller, and `rs.pipeline().start()` - used by `engine/streams.py`'s `ContinuousCapture`, which both `ThresholdPreviewThread` and `SessionEngineThread` construct - disrupts other devices on that same shared controller badly enough to knock the panel's USB-to-serial connection out, independent of how much stream bandwidth is actually configured. **Calibration never hits this**: it deliberately uses the lower-level, per-sensor `capture_synced_frame_pair` (raw `sensor.open()`/`.start()`) instead of `ContinuousCapture` - already documented above as a choice made for an unrelated reason (spurious zero-LED-detected results) - which incidentally also means Calibration's own LED-panel commands never race against a `rs.pipeline().start()` call at all, only Threshold Tuning's preview and Live Session do.

**The actual fix is physical, not a code change**: put the LED panel and the RealSense camera on separate USB controllers (e.g. a rear motherboard port for one, a front-panel/external hub for the other) - not two ports on the same hub. If a symptom matches this exactly (Calibration's on/off cycle works cleanly, but the very next `start_scanning()` - Threshold Tuning's preview or Live Session's Start - fails every single command with `LED-Panel.exe` itself reporting `"No device found."`, and retrying/restarting the app doesn't clear it), check USB topology before assuming a code or panel-firmware bug - this has nothing to do with the dual-panel arm-sequence/priming history documented below, which is a different failure class (the panel firmware's own internal state, not the OS losing the USB device entirely).
