...
Camera controls - `set_emitter_enabled(sensor, enabled)`, `set_manual_exposure(sensor, exposure)`, `enable_auto_exposure(sensor)`, all in `engine/streams.py` - are applied once PER DISTINCT RESOLVED SENSOR (i.e. once per `resolve_and_group` group), not once per stream pick, since two picks might share a sensor. Stream Config's UI presents ONE global "Camera Controls" group box (`gui/pages/stream_config_page.py`'s `_build_camera_control_group`, not per-resolved-sensor-group - an earlier per-group UI was simplified away, see `docs/algorithm_review_log.md`'s Issue 6): an IR-emitter-disable checkbox, ONE shared auto/manual exposure MODE radio pair, but **two independent exposure spinboxes** (Exposure A/Exposure B) underneath that shared mode - different sensors (IR vs RGB, or two different IR sensors) have different brightness characteristics, the same reasoning Threshold Tuning's own independent per-stream threshold fraction already uses. Read back as ONE `camera_controls` dict (`{emitter_enabled, auto_exposure, exposure_a, exposure_b}`, no `gain` key at all - see below) via `_read_camera_controls()` at "Next".

That single `camera_controls` dict is applied from four separate call sites, each of which re-derives `groups` via its own `resolve_and_group(device, pick_a, pick_b)` call: `gui/pages/roi_select_page.py`'s `_apply_camera_controls` (used by both ROI Select and, imported directly, by `gui/pages/calibration_page.py`), and `engine/session_engine.py`'s `SessionEngineThread.run()` and `engine/threshold_preview_thread.py`'s `ThresholdPreviewThread.run()` (each duplicated inline rather than imported, since these are hardware-thread files, not GUI code). Emitter enable and auto/manual MODE apply identically to every group (one shared choice), but in Manual mode, each call site calls `engine.streams.exposure_for_group(profiles, pick_a, pick_b, camera_controls["exposure_a"], camera_controls["exposure_b"])` per group to decide WHICH of the two exposure values that particular group actually gets - a group containing only pick_a's stream gets `exposure_a`, only pick_b's gets `exposure_b`. A group containing BOTH (the Dual-RGB shape - two stream profiles sharing one physical sensor) can only ever have ONE real exposure value in hardware regardless of what the UI offers per stream - `exposure_a` wins in that case, arbitrarily but deterministically (`exposure_for_group`'s own docstring).

**Manual exposure mode touches EXPOSURE ONLY - it deliberately never reads or writes GAIN at all.** This used to be different: `set_manual_exposure(sensor, exposure, gain)` wrote three options (`enable_auto_exposure=0`, `exposure`, `gain`), and `enable_auto_exposure` was responsible for restoring exposure/gain to the sensor's own `get_option_range().default` BEFORE switching auto back on, but ONLY when actually coming FROM manual (a "was-manual" gate - see below for why that gate itself exists). That restore-to-default approach was unit-tested and provably correct against a fake sensor, and it fixed a real observed bug: an earlier version that flipped only the auto flag left the manually-set gain (the UI defaulted it to 16) still written into the camera after a Manual->Auto round trip, dark enough that Calibration's Otsu blob detection stopped finding LEDs at all - and because the value lives in the CAMERA, not the app, it survived app restarts, so only a power-cycle/`hardware_reset` cleared it.

**That restore-to-default fix did not hold up on real hardware.** On the "RealSense D585 Prototype" rig specifically, the exact same symptom came back: gain not returning to its original value after Manual->Auto, still requiring a manual hardware reset - despite the restore logic being unit-tested and correct in isolation. The SDK-reported "factory default" this code restored to is firmware metadata, not a snapshot of the sensor's true pre-manual state; on this prototype board that reported value apparently isn't what a real hardware reset produces, and/or this sensor's own AE algorithm doesn't reliably re-take control of gain once it's been externally written, even after `enable_auto_exposure` flips back on. Trying to computationally restore gain to *a* value was fighting a firmware behavior that's proven unreliable here. The actual fix: `set_manual_exposure` no longer touches gain AT ALL - if our own code never writes it, there's nothing to restore and nothing for this class of bug to get stuck on. Switching back to Auto just re-enables the camera's own continuous AE algorithm, which drives gain live from the actual scene, the same as a real power-cycle would. The Stream Config UI's Gain spinbox was removed entirely (not just disabled) to match - `camera_controls` has no `"gain"` key at all now.

The was-manual gate (`sensor.get_option(rs.option.enable_auto_exposure) == 0`, checked BEFORE restoring) still matters for the exposure restore `enable_auto_exposure` does do: it's called unconditionally on EVERY apply point (ROI Select, Calibration, Threshold Tuning, Live Session) whenever "Auto exposure" is selected - not just on an actual Manual->Auto transition. An intermediate version restored the exposure default unconditionally every time, including when the sensor was ALREADY auto-exposing correctly - forcing a cold reset-and-reconverge on every single Calibration/ROI Select run, which could leave exposure under-converged within `calibration.settle_frames`' short window even though it would have stayed correctly exposed if left alone. Symptom on real hardware: scattered, intermittent LED detection dropouts on every run, not the original bug's total blackout - a regression that came back even when the operator never touched Manual exposure at all. Gating the restore on the sensor's OWN currently-reported state leaves an already-auto sensor completely undisturbed while still self-healing the actual Manual->Auto transition.

`tools/camera_diag/reset_camera_exposure.py` is a one-off, software-only alternative to a physical power-cycle for a sensor already stuck from before either of these fixes existed. Since the app's own runtime no longer touches gain at all, this diagnostic script does its own EXPLICIT, direct gain reset (`sensor.set_option(rs.option.gain, sensor.get_option_range(rs.option.gain).default)`) independent of `enable_auto_exposure` - its whole job is clearing out whatever a real sensor is CURRENTLY stuck with, including gain left over from before the app stopped touching it.

**`set_manual_exposure` clamps the given exposure to the sensor's own currently-reported valid range before writing it.** Confirmed on real hardware: applying Manual exposure raised a raw pyrealsense2 exception - `"out of range value for argument 'value'"` - straight out of `sensor.set_option()`. The Stream Config UI's exposure spinbox accepts any value from 1 to 1,000,000 with no way to know ahead of time what a given sensor's actual valid range is; that range isn't a fixed property of the sensor model, it can depend on the CURRENTLY configured resolution/fps. `set_manual_exposure` now reads `sensor.get_option_range(rs.option.exposure)` and clamps the requested value to `[range.min, range.max]` before calling `set_option` - a value too extreme for the current configuration still applies (at its nearest valid bound) instead of hard-failing the whole capture. This is independent of (and unrelated to) the gain fix above - it would have applied even under the old three-option `set_manual_exposure(sensor, exposure, gain)` signature.
...
This is a **manual, per-camera-FLOW operator choice**, not inferred from
the camera model, not auto-detected from the hub, and not a per-named-test
`settings.yaml` flag - Stream Config's "Use dual LED panel" checkbox
(`gui/pages/stream_config_page.py` - moved here from Device Select; see
that page's own comment on `dual_panel_checkbox` and the "Camera
management" section below for why) is read once in
`MainWindow._on_config_chosen` into `self._dual_panel_config` (`None` for
...
### Back navigation: safe per page's actual runtime behavior, not one generic rule

Every wizard page except Camera Hub (the wizard's root - nothing to go back to) has a `back_requested` signal and a Back button, wired in `gui/main_window.py`'s `MainWindow.__init__` straight to a `stack.setCurrentWidget(...)` call for that page's predecessor (`Camera Hub -> Device Select -> Stream Config -> ROI Select -> Calibration -> Threshold Tuning`, and `Live Session`/`Multi-Camera Live Session -> Camera Hub`). Back never calls `set_context()`/`populate()` again on the target page - whatever it already holds (previous picks, drawn ROI, calibration log, retuned thresholds) is shown exactly as left, no work redone.

What Back actually DOES before emitting differs per page, because each page's own runtime behavior differs - there is no one shared "stop everything" helper, since "everything" means something different (or nothing) on each page:

- **Device Select / ROI Select**: nothing persistent to stop (device listing and ROI capture are just synchronous calls); ROI Select's capture has no `processEvents()` at all, so `back_button` is disabled around it anyway as cheap, consistent insurance rather than because of a real reentrancy risk. (RGB Mode's own mode-switch reentrancy risk - the actual reason this bullet originally singled out Device Select - moved to Stream Config along with the control itself; see that page's own Back-disable handling below.)
- **Stream Config / Threshold Tuning**: Back silently calls the page's existing blocking preview-stop (`_stop_preview()` / `_stop_preview_blocking()`) before emitting - the exact same silent auto-stop "Next"/"Continue to Live Test" already do on these pages. No confirmation: a live preview has no operator work to lose. Stream Config's own `back_button`/`next_button`/`combo_test`/`mode_group_box` are additionally disabled for the span of an in-progress RGB Mode switch (`MainWindow._on_stream_config_mode_switch_requested` - see that section below) - genuinely reentrant, since that switch pumps `QApplication.processEvents()` while it runs.
- **Calibration**: Back is disabled for `_run_calibration`'s entire synchronous, `processEvents()`-pumped span (mirrors `run_button`'s own disable/enable) - there's no attempt to cancel a run partway through, only to prevent leaving mid-run.
- **Live Session / Multi-Camera Live Session**: unlike a preview, an actively running session is a deliberate, real timed test - interrupting it isn't obviously reversible the way stopping a preview is, so Back shows a `QMessageBox.question` confirmation first. On Yes, it calls the page's existing non-blocking stop (`stop_session()` / `stop_all_sessions()`) then blocks on the engine thread(s) actually finishing (`engine_thread.wait()`, or every thread in `MultiCameraSessionController.threads` for the multi-camera page) before emitting - the same "wait for the thread's own hardware cleanup, not just `request_stop()`" reasoning `start_session()`'s own defensive wait already uses, generalized here so hardware is genuinely free before the page is left. `back_button` is disabled for that blocking-wait span too, for the same cheap-insurance reasoning as ROI Select's above (the wait already blocks the event loop, so this isn't a real reentrancy fix). On No, nothing is touched and the session keeps running.

This same-feature-seven-times shape is deliberate, not an oversight that should be refactored into one shared helper: what "safe" means is genuinely different per page (nothing to stop vs. silently stop vs. disable-only vs. confirm-then-stop), so a shared abstraction would either paper over real differences or need per-page overrides anyway. Keep each page's Back handler colocated with (and consistent with) that page's own existing Stop/Next/Continue logic, not extracted elsewhere.

**Calibration's own forward path off of a Back visit: "Continue to Threshold Tuning" reuses the cached result instead of forcing a real re-run.** Real-world friction this fixed: Threshold Tuning's own Back returns to Calibration exactly as left (see above - no `set_context()` re-call), but the only way forward used to be `calibration_done`, which only ever fired from a full `_run_calibration()` - forcing the operator to redo an actual camera/LED-panel capture just to get back to the page they were already on. `continue_button` (enabled only once `last_calibration_result` holds a result for the CURRENT context) just re-emits the same `calibration_done` signal a real run does - `MainWindow._on_calibration_done` doesn't know or care which path produced it, since it re-derives everything else (LED positions) by reading back from `config.yaml`, which the earlier real run already saved via `update_config_leds`. `set_context()` resets `last_calibration_result`/disables `continue_button` on every call, since a fresh arrival means genuinely new pending args (different picks/ROI) that haven't been calibrated yet - Back never calls `set_context()` again, so this reset never fires on the actual Back-then-Continue path, only on a real forward re-arrival from ROI Select.
...
**Serial number shown everywhere a camera name is** - two identical-model cameras were otherwise indistinguishable in every UI spot that only showed the model name. `gui/pages/live_session_page.py`'s new `_camera_display_name(camera_name, device_serial)` (sibling to the existing `_short_camera_name`, duplicated the same way into `gui/widgets/camera_live_session_panel.py`) formats `"D455 [12345]"`, used by Threshold Tuning/Live Session/Multi-Camera panel titles. `MainWindow._refresh_camera_hub` uses the same `"{} [{}]"` format for Camera Hub cards, and `multi_camera_live_session_page.py`'s own `_camera_roles` (cross-camera section headers, static-export titles) was changed from its old `"(SN ...)"` format to the same bracket convention, for one consistent look everywhere.

**Device Select hides already-configured cameras from Add** - `DeviceSelectPage.refresh_devices()` gained `exclude_serials`; `MainWindow._on_add_camera_requested` passes every already-configured camera's serial, so the same physical camera can't be added to the test twice. Edit never calls `refresh_devices()` at all (see below), so there's no need to special-case "don't hide the one being edited."

**"Use dual LED panel" and RGB Mode both live on Stream Config, not Device Select - and both are per-camera-FLOW, not whole-test settings.** The dual-panel checkbox looked like a natural whole-test setting at first glance, but isn't: the SAME camera can need different panel setups for different tests (e.g. a D585 doing IR-vs-RGB needs two panels, the same D585 doing IR-vs-IR needs one) - it depends on which Test/pairing gets picked, which only happens on Stream Config, so that's where the checkbox lives too (manual, not auto-inferred from the test - see stream_config_page.py's own comment on `dual_panel_checkbox`). RGB Mode moved for a related but distinct reason: switching mode changes which streams the device exposes, which would invalidate whatever Test/Sensor Options Stream Config had already resolved - the switch has to happen (and refresh those lists) on the SAME page that shows them, not one page earlier where nothing downstream exists yet to invalidate. `StreamConfigPage._on_next_clicked` emits a `mode_switch_requested(target_mode)` signal instead of proceeding when the radio differs from the device's current mode (fetched fresh in `populate()` via `get_mode`/`find_device_by_serial`) - it does NOT advance, since the switch may invalidate the current picks. `MainWindow._on_stream_config_mode_switch_requested` applies the switch (`ensure_mode`) then calls the same `_populate_stream_config_page()` helper `_on_device_chosen` uses, passing the CURRENT pick/test/dual-panel selections as `preferred_*` so anything unaffected by the mode change survives the refresh - anything genuinely invalidated just falls back to the first available option (`populate()`'s own existing tolerant fallback for any `preferred_*` that no longer matches). Device Select is now a pure device picker: pick one, click Next, `device_chosen` fires immediately - its combo labels still show each device's current mode as an informational suffix (`_device_label`, unchanged), just with no control left to change it.

**Edit jumps straight to Stream Config, skipping Device Select entirely, prefilled with that camera's own previous choices.** The device is already known for an existing camera, so re-picking it would be pointless - and would force Device Select's own `exclude_serials` to special-case not hiding the very camera being edited. `_on_edit_camera_requested` calls `_populate_stream_config_page()` directly with that camera's stored `device_serial`/label, passing `preferred_a`/`preferred_b`/`preferred_test_name`/`preferred_dual_panel`/`preferred_camera_controls` all read from that camera's own committed config - a genuine "continue editing", not "redo from scratch". `test_name` is a NEW field needed only for this prefill, stored as a **sibling** of each committed camera's `"config"` dict in `self._cameras[camera_id]`, not inside it - `"config"` is `**`-splatted directly into `LiveSessionPage.set_context()` (the 1-configured-camera path in `_on_start_multi_camera_session_requested`), which has no `test_name` parameter; read live off `stream_config_page.current_test_name` at `_on_tuning_done` commit time (unchanged since Stream Config ran, earlier in that same sub-flow). `StreamConfigPage.populate()` gained a matching `preferred_camera_controls` param (a new `_apply_camera_controls_to_widgets()`, the inverse of the existing `_read_camera_controls()`) and a `DEFAULT_CAMERA_CONTROLS` constant applied explicitly on a fresh Add - same stale-carryover reasoning as `preferred_dual_panel`'s own explicit reset, since this page's one instance is reused across every camera's own sub-flow visit.
