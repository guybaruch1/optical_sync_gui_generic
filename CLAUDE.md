...
**`capture_synced_frame_pair`'s (`engine/streams.py`) settle-frame timeout must scale with the stream's own fps, not be a flat constant.** Confirmed on real hardware: Calibration on a `RealSense D585 Prototype` color stream configured at 640x480@5fps with Manual exposure selected raised `RuntimeError: Timed out (waiting for post-trigger settled frames) - ...` - a fixed `timeout_s=10.0` default left almost no real-world margin at 5fps, where `calibration.settle_frames`'s default of 15 fresh frames needs a bare THEORETICAL MINIMUM of 15/5 = 3 seconds even with zero jitter (each dropped/delayed frame costs a full 200ms slot at 5fps vs. 33ms at 30fps, so the same amount of real-world startup/settle latency eats a much bigger fraction of the budget the slower the stream runs) - and hitting the timeout aborts the capture (and the sensor's in-progress manual-exposure transition) mid-settle via the function's own `finally: sensor.stop(); sensor.close()`, rather than letting an otherwise-healthy slow stream actually finish.

The fix: `timeout_s` defaults to `None` now, and `_default_capture_timeout_s(groups, settle_frames)` derives the real default from `settle_frames / (slowest requested stream's own fps) * _CAPTURE_TIMEOUT_SAFETY_FACTOR` (5.0), floored at the original fixed `_MIN_CAPTURE_TIMEOUT_S` (10.0) - so every stream at ~15fps or faster keeps the exact old fixed-10s behavior byte-for-byte (confirmed via `test_default_capture_timeout_floors_at_the_original_constant_for_fast_streams`), and only a genuinely slow stream (5fps -> 15s) gets a bigger budget. `_try_min_requested_fps` follows the same opaque-test-fake-profile fallback convention `_try_derive_expected_keys`/`_try_get_stream_key` already use (`getattr(profile, "fps", None)` + `callable()` check) - falls back to the fixed floor when fps can't be determined, so this can only ever grow the timeout relative to the old behavior, never shrink it. An explicit `timeout_s=` argument (only ever used by this file's own tests, to force short timeouts) still always wins - the fps-derived default only ever kicks in when the caller doesn't pass one, which every production call site (ROI Select, Calibration, `tools/panel_drift/panel_drift_calibrate.py`) already does.

**`set_manual_exposure` clamps the given exposure to the sensor's own currently-reported valid range before writing it.** Confirmed on real hardware: applying Manual exposure at ROI Select on the same 640x480@5fps color stream raised a raw pyrealsense2 exception - `"out of range value for argument 'value'"` - straight out of `sensor.set_option()`. The Stream Config UI's exposure spinbox accepts any value from 1 to 1,000,000 with no way to know ahead of time what a given sensor's actual valid range is; that range isn't a fixed property of the sensor model, it can depend on the CURRENTLY configured resolution/fps. `set_manual_exposure` now reads `sensor.get_option_range(rs.option.exposure)` and clamps the requested value to `[range.min, range.max]` before calling `set_option` - a value too extreme for the current configuration still applies (at its nearest valid bound) instead of hard-failing the whole capture. This is independent of (and unrelated to) the gain fix above - it would have applied even under the old three-option `set_manual_exposure(sensor, exposure, gain)` signature, it just hadn't been hit until this rig's first-ever attempt at Manual exposure on this specific slow-fps configuration.
...
**Calibration's debug detection images (`domain/realsense_utils.py`'s `save_debug_detection_image`) are drawn at 4x the native cropped-ROI resolution (`_DEBUG_IMAGE_SCALE`), not at native size.** A dense LED grid (e.g. 100 LEDs) cropped to its own ROI is only a few pixels per LED - nowhere near enough to render a 1-3 digit `led_id` legibly at any font scale, and naive zooming in an image viewer just blurs/pixelates a PNG that never had the resolution to begin with; confirmed on real hardware when trying to visually compare LED numbering direction between two streams' debug images (checking whether `assign_grid_ids`' row-major numbering assumption - each stream numbered completely independently, purely from that stream's own photo geometry, with no code anywhere cross-checking that LED id N means the same physical LED in both streams - actually agrees between them, part of an ongoing investigation into a large, sawtooth-shaped `position_gap_ms`/"Optical Sync" artifact seen despite `pairing_gap_us`/"HW TS Latency" reading correctly near 0) and being unable to read the printed numbers even zoomed in. The whole base image is upscaled via `cv2.resize(..., interpolation=cv2.INTER_NEAREST)` before anything is drawn - nearest-neighbor, not a smoothing interpolation, since this is a verification image where crisp original LED-blob edges matter more than a smooth-looking scale-up - and every drawn coordinate/radius/font-scale/text-thickness is scaled by the same factor so circles and numbers grow proportionally with the canvas. `draw_detected_centroids` (the separate, no-file-write circles-only variant Threshold Tuning's live retuning preview uses, drawn on top of an already-reasonably-sized live video panel) is deliberately NOT scaled the same way - a different legibility situation with its own layout constraints.

**The 4x upscale alone was not enough - each label's fixed offset-to-the-right, fixed-font-scale placement made a dense grid's labels bleed into their neighbors.** Confirmed on real hardware even after the 4x fix above: on the actual 10x10/100-LED panel this app targets, LEDs are only ~48px apart at 4x scale, but a 2-3-digit `led_id` at the original fixed font scale renders noticeably wider than that gap - `led_id` N's label physically overlaps `led_id` N+1's own circle and label (drawn moments later, so it visually wins), garbling long, unbroken runs of adjacent labels into an unreadable smear. The operator still could not reliably read individual ids off the resulting image, which is exactly the thing the 4x fix was supposed to enable. The actual fix: `save_debug_detection_image` now draws each `led_id` **centered inside its own detected LED's circle** rather than offset into the gap to its right, at a font scale individually fit (`_fit_label_to_box`, shrinking from a starting scale down to a floor) to stay within that LED's own real measured spacing (`_typical_spacing`, not the decorative, separately-capped circle radius) - a placement that structurally cannot reach into a neighboring LED's own cell no matter how many digits `led_id` has or how dense the grid is, so it needed no per-density tuning to get right. `tests/domain/test_realsense_utils.py`'s `test_save_debug_detection_image_labels_never_collide_on_a_dense_grid` locks this in directly: it renders a synthetic 10x10 grid at LED spacing tight enough that the OLD fixed-offset placement would provably collide, then asserts every label's red pixels stay within its own cell's boundary.
