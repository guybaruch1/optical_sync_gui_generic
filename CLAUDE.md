...
- **Device Select / ROI Select**: nothing persistent to stop (device listing and ROI capture are just synchronous calls), but both pages' own primary action (`ensure_mode`'s mode switch, `capture_synced_frame_pair`'s capture) pumps `QApplication.processEvents()` while running - Device Select's mode switch genuinely can be reentered this way, so Back is disabled for that span exactly like `next_button`/`combo`/`mode_group_box` already are; ROI Select's capture has no `processEvents()` at all (so this can't actually happen there), but `back_button` is disabled around it anyway as cheap, consistent insurance.
- **Stream Config / Threshold Tuning**: Back silently calls the page's existing blocking preview-stop (`_stop_preview()` / `_stop_preview_blocking()`) before emitting - the exact same silent auto-stop "Next"/"Continue to Live Test" already do on these pages. No confirmation: a live preview has no operator work to lose.
